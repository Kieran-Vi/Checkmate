...
const escalationSchema = z
	.object({
		...
		levels: z
			.array(
				z.object({
					delayMinutes: z.number().min(0, "Delay must be a non-negative number"),
					address: z.string().min(1, "Escalation address is required").email("Please enter a valid e-mail address"),
				})
			)
